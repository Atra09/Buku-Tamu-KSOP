      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-sky-100 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-sky-600 text-white px-5 py-3.5 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-2">
            <CheckCircle className="w-5 h-5 text-sky-100" />
            <h3 className="font-bold text-base tracking-tight">Kartu Kunjungan Tamu</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto" ref={printRef}>
          <div className="border-2 border-dashed border-sky-300 rounded-xl p-5 bg-gradient-to-b from-sky-50/60 to-white text-center shadow-inner">
            <div className="flex flex-col items-center justify-center mb-3">
              <div className="relative inline-block text-center">
                <img
                  src="/images/kementrianperhubungan.png"
                  alt="Logo Kemenhub"
                  className="absolute right-full mr-3 top-1/2 -translate-y-1/2 w-9 h-9 object-contain shrink-0"
                  onError={(e) => {
                ...
                />
                <h4 className="font-black text-slate-800 text-sm sm:text-base tracking-tight leading-tight">
                  Si-Tamu • KEMENTERIAN
                </h4>
                <h4 className="font-black text-slate-800 text-sm sm:text-base tracking-tight leading-tight">
                  PERHUBUNGAN
              ...
              <p className="text-[11px] text-sky-800 font-extrabold uppercase tracking-widest mt-1 text-center">
            ...
            {guest.foto ? (
              <img
                ...
                alt={guest.nama}
                className="w-64 h-64 aspect-square mx-auto rounded-2xl object-cover border-4 border-sky-600 shadow-lg mb-4"
              />
            ) : (
            ...
            <div className="bg-sky-600 text-white py-2 px-4 rounded-xl font-black text-base tracking-widest mb-4 shadow-xs text-center">
              {formatDisplayNoReg(guest.no_reg)}
            </div>

            {/* Details Table */}
            <div className="text-left space-y-2.5 text-xs">
              <div className="flex items-start gap-2 border-b border-sky-100 pb-1.5">
              ...
              <div className="flex items-start gap-2 border-b border-sky-100 pb-1.5">
                <Tag className="w-4 h-4 text-sky-600 shrink-0 mt-0.5" />
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Bertemu & Keperluan</span>
                  <span className="font-bold text-sky-950 block">{guest.bertemu}</span>
                  <span className="text-slate-600">{guest.keperluan}</span>
                </div>
              </div>

              <div className="space-y-1 text-[11px] text-slate-600 pt-2 border-t border-sky-100">
        ...
        <div className="bg-slate-50 px-5 py-3 border-t border-slate-200 flex items-center justify-end gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-xs font-bold bg-sky-600 hover:bg-sky-700 text-white rounded-lg shadow-sm flex items-center gap-1.5 transition-all cursor-pointer"
          >
